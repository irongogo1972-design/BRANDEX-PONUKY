...
if 'client' not in st.session_state: 
    st.session_state['client'] = {"firma": "", "adresa": "", "osoba": "", "platnost": datetime.now() + timedelta(days=14), "vypracoval": ""}
...
<style>
    ...
    .header img {{ width: 220px; }} /* LOGO NA 220PX */
    .main-title {{ font-size: 32px; font-weight: bold; text-align: center; text-transform: uppercase; margin: 0 0 15px 0; }}
    ...
    /* TABUĽKA */
    .items-table {{ width: 100%; border-collapse: collapse; margin-top: 10px; color: black; }}
    ...
    .img-cell img {{ max-width: 75px; max-height: 100px; object-fit: contain; }}
    ...
    .graphic-col {{ width: 48%; display: flex; flex-direction: column; gap: 10px; }}
    .graphic-box {{ border: 1px dashed #ccc; padding: 10px; text-align: center; min-height: 110px; }}
    .graphic-box img {{ max-width: 100%; max-height: 130px; margin-bottom: 10px; display: block; margin-left: auto; margin-right: auto; }}
...
with st.sidebar:
    ...
    with st.expander("👤 Odberateľ", expanded=False):
        c_firma = st.text_input("Názov firmy", "")
...
sum_vat_base = total_items + total_brand
...
<div class="paper">
    ...
    <table class="items-table">
        ...
        <tbody>
            {table_body if table_body else "<tr><td colspan='10'>Žiadne položky</td></tr>"}
        </tbody>
    </table>
    ...
    <div class="branding-row">
        ...
        <div style="grid-column: span 1"><b>Popis</b><br>{b_desc if b_desc else "..."}</div>
...
# VYKRESLENIE
st.markdown(final_document, unsafe_allow_html=True)
